# Preliminary Kenya Analysis to Convert to R Markdown File - LAST ANTIBIOTIC USE
# SAB 12/5/2023
...
sample_data(ps)$LastABXUse <- factor(sample_data(ps)$LastABXUse,
                                                      levels = c("Last year", "Last month", "This month", "This week"))
...
psbclass %>% plot_composition(group_by = "LastABXUse") +
...
mps <- merge_samples(psrel, "LastABXUse")
...
ggviolin(ps.meta, x = "LastABXUse", y = "Shannon$Shannon",
         add = "boxplot", fill = "LastABXUse", palette = four_color, title = " ", ylab = "Shannon's Diversity Index", xlab = " ") +
  theme(legend.position = "none") +
...
ait %>% dist_bdisp(variables = "LastABXUse") %>% bdisp_get()

# test with PERMANOVA
mod1 <- ait %>%
  dist_permanova(
    seed = 81299,
    variables = "LastABXUse",
    n_perms = 9999
  )
...
ait %>% 
  ...
  ord_plot(color = "LastABXUse", size = 6, axes = c(1,2)) +
