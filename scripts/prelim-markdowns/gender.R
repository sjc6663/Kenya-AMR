...
color_palette <- c("#45337d", "#2e6f8f", "#218f8c", "#1fa187", "#2db27d", "#83a561")
...
#ggsave(A, file = "bovine-host-resistome/plots/gender-relabund.tiff", dpi = 600)
...
mod <- lm(Shannon ~ MF + Size + MF*Size, data = adiv)
...
B <- plot_richness(ps, x="OperatorGender", measures=c("Shannon"), color = "OperatorGender") + 
  geom_boxplot() + 
  geom_jitter() +
...
#ggsave(B, file = "bovine-host-resistome/plots/gender-alpha.tiff", dpi = 600)
