# Preliminary Kenya Analysis to Convert to R Markdown File - MILKINGS PER DAY
# SAB 12/5/2023
...
sample_data(psbclass)$MilkingsPerDay <- factor(sample_data(psbclass)$MilkingsPerDay,
                                                      levels = c("1", "2", "3"))


psbclass %>% plot_composition(group_by = "MilkingsPerDay") +
...
mps <- merge_samples(psrel, "MilkingsPerDay")
...
adiv <- data.frame(
  ...
  "milkings" = phyloseq::sample_data(ps)$MilkingsPerDay
)

mod <- lm(Shannon ~ milkings, data = adiv)
summary(mod)

sample_data(ps)$MilkingsPerDay <- factor(sample_data(ps)$MilkingsPerDay,
                                               levels = c("1", "2", "3"))
...
ggviolin(ps.meta, x = "MilkingsPerDay", y = "Shannon$Shannon",
         add = "boxplot", fill = "MilkingsPerDay", palette = four_color, title = " ", ylab = "Shannon's Diversity Index", xlab = " ") +
  theme(legend.position = "none") +
...
ait %>% dist_bdisp(variables = "MilkingsPerDay") %>% bdisp_get()

# test with PERMANOVA
mod1 <- ait %>%
  dist_permanova(
    seed = 81299,
    variables = "MilkingsPerDay",
    n_perms = 9999
  )
...
ait %>% 
  ...
  ord_plot(color = "MilkingsPerDay", size = 6, axes = c(1,2)) +
  scale_color_manual(values = four_color) +
  stat_ellipse(aes(group = MilkingsPerDay, color = MilkingsPerDay)) + 
  theme_classic() +
  labs(color = "Milkings ") +
  ggtitle(" ") + 
  labs(caption = " ") +
