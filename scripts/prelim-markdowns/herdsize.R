# Preliminary Kenya Analysis to Convert to R Markdown File - HERD SIZE
# SAB 12/5/2023
...
# create bins for herd size as less than 10 or more than 10
ps <- ps %>% 
  ps_mutate(
    size = if_else(sample_data(ps)$NumberCows>10, true = ">10", false = "<10")) 
...
psbclass %>% plot_composition(group_by = "size") +
...
adiv <- data.frame(
  ...
  "Size" = phyloseq::sample_data(ps)$size,
...
ait %>% dist_bdisp(variables = "size") %>% bdisp_get()
...
ait %>% 
  ...
  ord_plot(color = "size", size = 6, axes = c(1,2)) +
  scale_color_manual(values = short_color) +
  stat_ellipse(aes(group = size, color = size)) + 
  theme_classic() +
...
# run AlDEx2 function
aldex2 <- ALDEx2::aldex(data.frame(phyloseq::otu_table(ps)), phyloseq::sample_data(ps)$size, taxa_rank = "all", norm = "CLR", method = "t.test", p_adjust = "BH", pvalue_cutoff = 0.05, mc_samples = 128, denom = "iqlr")
